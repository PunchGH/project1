...
def QuesAndAns(test_level):
    ...
    user_ans = Entry(exam_window,fg=color_font.tx_color,bg=color_font.bg_color,font=color_font.font)
    user_ans.grid(column=1,row=1)
    user_ans = user_ans.get()
    
    def submit_button():
        
        if ans == user_ans:
            
            correct_window =Toplevel()
            
            Label(correct_window,text="Correct",bg=color_font.bg_color,fg=color_font.tx_color).grid(row=0,column=0)
    
    submit_button = Button(exam_window,text="submit",bg=color_font.bg_color,fg=color_font.tx_color,font=color_font.font,command=submit_button)
